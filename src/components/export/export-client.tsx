...
import type { PatentWithRelations, PriorArtResult, SectionType } from "@/lib/types";
...
export function ExportClient({ patent, priorArtResults }: ExportClientProps) {
  ...
  async function handleDownload(
    ...
    try {
      const res = await fetch(
        `/api/export/${format}?patentId=${patent.id}`
      );
  ...
  async function handleDownloadIds() {
    ...
    try {
      const res = await fetch(
        `/api/export/ids?patentId=${patent.id}`
      );
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="flex items-center gap-2 text-sm py-1">
              ...
              <span className={patent.claims.length > 0 ? "" : "text-muted-foreground"}>
                Claims
              </span>
              {patent.claims.length > 0 && (
                <Badge variant="secondary" className="ml-auto text-xs">
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="paragraphNumbers" className="text-sm cursor-pointer">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="headersFooters" className="text-sm cursor-pointer">
        ...
        <Card className="flex flex-col">
          ...
          <CardContent>
            <Button
              className="w-full"
              variant="outline"
              disabled={patent.drawings.length === 0}
            >
